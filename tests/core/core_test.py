...
class CoreUnitTest(unittest.TestCase):
    def test_CanCalculateLifetime(self):
        ...
        delta_tau_mock.calculate_error_propagation_terms.return_value = np.array([0.6, 0.2])
        ...
        with patch.dict(
            ...
            np.testing.assert_array_equal(chi_mock.optimize_t_hyp.mock_calls[0].args[0],
                                          np.array([2, 6]))

            np.testing.assert_array_equal(chi_mock.optimize_t_hyp.mock_calls[0].args[1],
                                          np.array([6, 10]))

            np.testing.assert_array_equal(chi_mock.optimize_t_hyp.mock_calls[0].args[2],
                                          np.array([1, 1]))

            np.testing.assert_array_equal(chi_mock.optimize_t_hyp.mock_calls[0].args[3],
                                          np.array([1, 1]))

            np.testing.assert_array_equal(chi_mock.optimize_t_hyp.mock_calls[0].args[4],
                                          np.array([1, 1, 1]))

            np.testing.assert_array_equal(chi_mock.optimize_t_hyp.mock_calls[0].args[5],
                                          np.array([0, 1]))

            self.assertEqual(chi_mock.optimize_t_hyp.mock_calls[0].args[6],
                                          (-5, 5))

            self.assertEqual(chi_mock.optimize_t_hyp.mock_calls[0].args[7],
                                          1.0)
            ...
            self.assertEqual(
                chi_mock.optimize_coefficients.mock_calls[0].args[7],
                1.0
            )

            self.assertEqual(len(tau_mock.calculate_tau_i_values.mock_calls), 1)

            np.testing.assert_array_equal(tau_mock.calculate_tau_i_values.mock_calls[0].args[0],
                                          np.array([2, 6]))

            np.testing.assert_array_equal(tau_mock.calculate_tau_i_values.mock_calls[0].args[1],
                                          np.array([6, 10]))

            np.testing.assert_array_equal(tau_mock.calculate_tau_i_values.mock_calls[0].args[2],
                                          np.array([1, 1]))

            np.testing.assert_array_equal(tau_mock.calculate_tau_i_values.mock_calls[0].args[3],
                                          np.array([1, 1]))

            np.testing.assert_array_equal(tau_mock.calculate_tau_i_values.mock_calls[0].args[4],
                                          np.array([1, 1, 1]))

            np.testing.assert_array_equal(tau_mock.calculate_tau_i_values.mock_calls[0].args[5],
                                          np.array([0, 1]))

            self.assertEqual(tau_mock.calculate_tau_i_values.mock_calls[0].args[6],
                             (-5, 5))

            self.assertEqual(tau_mock.calculate_tau_i_values.mock_calls[0].args[7],
                             1.0)

            self.assertEqual(tau_mock.calculate_tau_i_values.mock_calls[0].args[8],
                             None)

            self.assertEqual(len(delta_tau_mock.calculate_error_propagation_terms.mock_calls), 1)
