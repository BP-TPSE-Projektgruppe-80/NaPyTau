...
def evaluate_polynomial_at_measuring_distances(
    distances: ndarray, relative_velocity: float, coefficients: ndarray
) -> ndarray:
    """
    ...
    sum_at_measuring_distances: ndarray = zeros_like(times, dtype=float)
    for exponent, coefficient in enumerate(coefficients):
        sum_at_measuring_distances += coefficient * power(times, exponent)
    ...
    for exponent, coefficient in enumerate(coefficients):
        if exponent > 0:
            sum_of_derivative_at_measuring_distances += (
                exponent * coefficient * power(times, (exponent - 1))
            )
